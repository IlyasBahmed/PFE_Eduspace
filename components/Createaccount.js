...
export default function Createaccount() {
    ...
    const handlePage = async () => {
        ...
        if (user.trim() === "") {
            setError("Veuillez remplir le nom d'utilisateur");
        } else if (email.trim() === "") {
            ...
            setError(""); 
            try {
                const response = await axios.post("http://localhost:6005/createAccount", {
                    name: user,
                    email: email,
                    password: passw,
                });
    ...
    return (
        <View style={styles.container}>
            ...
            <Button text={"Create Account"} onPress={handlePage} />
